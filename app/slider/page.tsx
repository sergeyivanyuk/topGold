...
export default function SliderPage() {
	...
	const slides = constants.tariffs.map(tariff => ({
		id: tariff.id,
		icon: tariff.icon,
		title: tariff.name,
		goldAmount: tariff.goldAmount,
		bonusGold: tariff.bonusGold,
		bonusText: tariff.bonusText,
		chanceText: tariff.chanceText,
		price: `${tariff.price}₽`,
		buttonText: 'Выбрать'
	}))
	...
	return (
		<main className="flex flex-col items-center justify-start p-4 overflow-x-hidden">
			{/* Заголовок страницы */}
			<h1 className="text-2xl font-bold mb-1 text-white">Покупка голды</h1>
			<p className="mb-10 text-gray-subtext">Каждая покупка - гарантированная голда</p>
			...
			<div
				...
				role="region"
				aria-label="Тарифы на покупку голды"
			>
						...
						return (
							...
							>
								<TariffCard
									icon={slide.icon}
									title={slide.title}
									goldAmount={slide.goldAmount}
									bonusGold={slide.bonusGold}
									bonusText={slide.bonusText}

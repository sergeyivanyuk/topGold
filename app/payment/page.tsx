...
function PaymentContent() {
	...
	const selectedTariff = constants.tariffs.find(t => t.name === tariff) || constants.tariffs[0]
	const goldAmount = selectedTariff.goldAmount
	const bonusGold = selectedTariff.bonusGold
	const goldText = `${goldAmount.toLocaleString('ru-RU')} голды`
	const bonusText = bonusGold > 0 ? `+${bonusGold.toLocaleString('ru-RU')} бонусной голды` : null
	...
	return (
		<main className=" flex flex-col items-center justify-start p-4">
			...
			<div className="relative w-full text-center">
				...
				<h1 className="text-2xl font-bold mb-1 text-white">Оплата тарифа</h1>
			</div>
			...
			<div className="w-full rounded-card overflow-hidden bg-[url('/banner.png')] bg-cover bg-center p-5 pb-10 mb-5">
				<h3 className="text-gold-title text-[22px] mb-2.5">Что вы получите</h3>
				<div className="flex flex-col gap-1">
					<div className="flex items-center gap-2">
						<span className="block w-2 h-2 rounded-full bg-bullet-gradient"></span>
						<span className="text-guaranteed-reward">Гарантированная выплата</span>
					</div>
					<div className="flex items-center gap-2">
						<span className="block w-2 h-2 rounded-full bg-bullet-gradient"></span>
						<span className="text-guaranteed-reward">{goldText}</span>
					</div>
					{bonusText && (
						<div className="flex items-center gap-2">
							<span className="block w-2 h-2 rounded-full bg-bullet-gradient"></span>
							<span className="text-guaranteed-reward">{bonusText}</span>
						</div>
					)}
				</div>
			</div>
			...
			<div className="mt-7.5 grid grid-cols-2 gap-2.5 w-full max-w-4xl">
				...
				<div className="flex flex-col items-center justify-end  p-4 rounded-card bg-radial-gold border-gold-light outline-offset-[-1px]">
					...
					<div className="self-stretch h-0 opacity-40 outline-1 outline-offset-[-0.50px] outline-orange-200 my-2" />
					<span className="text-center text-secondary-title">В течении 24 часов</span>
				</div>
			</div>

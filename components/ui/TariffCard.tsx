...
export function TariffCard({
	...
	const totalGold = goldAmount + (bonusGold || 0)
	const formattedGold = totalGold.toLocaleString()
	const mainText = `${formattedGold} голды`
	...
	return (
		...
		>
			...
			{/* Основной текст (количество голды) */}
			<p className="text-slide-main mb-6">{mainText}</p>
			{/* Бонус (если есть) */}
			{bonusText && <p className="py-2.5 w-full text-center bg-gradient-bonus text-slide-bonus">{bonusText}</p>}
